-----------------------------------------------------------------------------------------------------
-------------------------------- REPLICAABCALTDIAECOMMNOVA - VENDA ECOMMERC   -----------------------
-----------------------------------------------------------------------------------------------------

select /*+OPTIMIZER_FEATURES_ENABLE('10.2.0.4')*/  
CASE WHEN NVL(V.NROFORMAPAGTO, -1) IN (68) AND V.nrosegmento <> 8 THEN 'SUP. NOW'
  WHEN NVL(V.NROFORMAPAGTO, -1) IN (73) AND V.nrosegmento <> 8 THEN 'IFOOD'
    WHEN NVL(V.NROFORMAPAGTO, -1) NOT IN (68, 73) AND V.nrosegmento = 8 THEN 'DELIVERY'
      ELSE NULL END ORIGEM,
...
from MRL_CUSTODIA Y, MAXV_ABCDISTRIBBASE V, MAX_COMPRADOR CP, MAP_PRODUTO A, MAP_PRODUTO PB, MAP_FAMDIVISAO D, MAP_FAMEMBALAGEM K, MAX_EMPRESA E, MAX_DIVISAO DV, MAP_PRODACRESCCUSTORELAC PR, MRLV_DESCONTOREGRA RE,
...
and ((NVL(V.NROFORMAPAGTO, -1) in ( 68, 73 )
or  V.CODGERALOPER = 517))
